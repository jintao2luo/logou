...
class LagouSpiderSpider(scrapy.Spider):
    ...
    def start_requests(self):
        ...
        yield scrapy.FormRequest(url, formdata={'first':'true', 'pn':'1', 'kd':'python'}, method='Post', meta={'pn':1}, headers=self.headers, callback=self.parse)
    ...
    def parse(self, response):
        ...
        if data:
            ...
            if pn <= pages:
                yield scrapy.FormRequest(response.url, formdata={'first': 'False', 'pn': str(pn), 'kd': 'python'}, method='Post', meta={'pn': pn}, headers=self.headers, callback=self.parse)
